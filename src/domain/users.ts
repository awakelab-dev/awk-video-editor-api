...
export function getUserTokenVersion(user: Pick<UserDocument, 'tokenVersion'>): number {
  return Number.isSafeInteger(user.tokenVersion) && Number(user.tokenVersion) >= 0
...
export function validateStrongPassword(password: string, email?: string, username?: string): ValidationError[] {
  ...
  if (email) {
    ...
    if (emailPart && emailPart.length >= 3 && lower.includes(emailPart)) {
      errors.push({ field: 'password', message: 'Password must not contain your email' })
    }
  }
  if (username) {
    const normalizedUsername = username.trim().toLowerCase()
    if (normalizedUsername.length >= 3 && lower.includes(normalizedUsername)) {
      errors.push({ field: 'password', message: 'Password must not contain your username' })
    }
  }
  if (/(.)\1{3,}/.test(password)) {
    errors.push({ field: 'password', message: 'Password must not contain repeated character sequences' })
  }
  if (/1234|2345|3456|4567|5678|6789|abcd|qwer|asdf/i.test(password)) {
    errors.push({ field: 'password', message: 'Password must not contain obvious sequences' })
  }
  return errors
}
...
export function validateRegisterPayload(body: unknown): ValidationError[] {
  ...
  if (typeof body.password !== 'string') {
    ...
    errors.push(...validateStrongPassword(
      body.password,
...
export function validateAdminUserPatchPayload(body: unknown): ValidationError[] {
  ...
  if (body.status !== undefined && body.status !== 'active' && body.status !== 'disabled') {
    errors.push({ field: 'status', message: 'Status must be active or disabled' })
  }
  if (body.role !== undefined && body.role !== 'admin' && body.role !== 'editor' && body.role !== 'viewer') {
    errors.push({ field: 'role', message: 'Role must be admin, editor, or viewer' })
  }
  if (Object.keys(body).length === 0) errors.push({ field: 'body', message: 'At least one field must be provided' })
  return errors
}
