...
import { UserDocument, generateUserId, getUserTokenVersion, toApiUser, validateLoginPayload, validateRegisterPayload } from '../domain/users'
import { env } from '../config/env'
import { authenticateRequest, AuthenticatedRequest } from '../middleware/authenticate'
...
function issueAccessToken(user: UserDocument): string {
  return jwt.sign(
    { sub: user.id, email: user.email, role: user.role, username: user.username, tokenVersion: getUserTokenVersion(user) },
    env.JWT_SECRET,
    {
...
router.post('/register', async (req: Request, res: Response) => {
  try {
    ...
    return res.status(201).json({ success: true, message: 'User registered successfully', data: { user: toApiUser(user) } })
  } catch (error: any) {
...
router.get('/me', authenticateRequest, async (req: AuthenticatedRequest, res: Response) => {
  return res.status(200).json({ success: true, message: 'Authenticated user fetched successfully', data: { user: toApiUser(req.user!) } })
})
