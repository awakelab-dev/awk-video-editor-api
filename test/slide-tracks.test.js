...
function buildTestApp(mockSummarizer) {
  clearDistCache()
  const { createSlideTracksRouter } = require('../dist/routes/slideTracks.js')
  const app = express()
  app.use(express.json({ limit: '2mb' }))
  app.use('/api/v1', createSlideTracksRouter(mockSummarizer))
  return app
}
...
test('POST /api/v1/slide-tracks returns generated tracks', async () => {
  ...
  const textTrack = response.body.data.tracks.find((track) => track.id === 'track_text')
  assert.ok(textTrack)
  ...
  assert.equal(textTrack.elements.some((element) => String(element.text).includes('\n')), true)
